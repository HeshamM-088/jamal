...
const PartOfCollection = ({ movieid }) => {
  const { movieDetailsdata } = useSelector((state) => state.moviesDetails);

  // const collectionsid = movieDetailsdata?.belongs_to_collection?.id.toString();
  ...
  useEffect(() => {
    if (movieDetailsdata?.belongs_to_collection?.id.toString()) {
      const collectionsid =
        movieDetailsdata?.belongs_to_collection?.id.toString();
